...
class EasyActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Toast.makeText(this, "Du valde lätt", Toast.LENGTH_SHORT).show()
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@EasyActivity, "Failed to fetch token", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
    private suspend fun fetchTriviaQuestions() {
        ...
        val url = "https://opentdb.com/api.php?amount=5&category=11&difficulty=easy&type=boolean&token=$token"
        val client = OkHttpClient()
        ...
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                e.printStackTrace()
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@EasyActivity, "Failed to fetch question", Toast.LENGTH_SHORT).show()
                }
            }
